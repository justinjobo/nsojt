...
-type cb_action() :: cb_action_act() | cb_action_cmd() | cb_action_init().
...
-type cb_action_init() ::
        fun((U::#confd_user_info{}, EconfdOpaque::term()) ->
                ...
                | {'ok', #confd_user_info{}}
                | {'error', error_reason()}).
%% It is the callback for #confd_action_cb.init If the action should
%% be done in a separate socket, the call to
%% econfd:new_worker_socket/3 must be done here. The worker and its
%% socket will be closed after the cb_action() returns.
...
%% action request. If a new worker socket was setup in the
%% cb_action_init that socket will be closed when the callback returns.
...
%% @doc Create a new worker socket to be used for an action
%% invocation.  When the action invocation ends
%% remove_worker_socket/1 should be called.
-spec new_worker_socket(UserInfo :: #confd_user_info{},
                        SockId :: integer()) ->
          {gen_tcp:socket() | ssl:socket(), #confd_user_info{}} |
          {'error', 'timeout' |
           'closed' |
           'not_owner' |
           'badarg' |
           inet:posix() |
           any() %% ssl can return {error, any()}
          }.
new_worker_socket(UserInfo, SockId) ->
    Actx = UserInfo#confd_user_info.actx,
    Dx = Actx#confd_action_ctx.dx,
    Host = Dx#confd_daemon_ctx.ip,
    Port = Dx#confd_daemon_ctx.port,
    ClientType = econfd_internal:client_type(),
    case econfd_internal:connect(Host, Port, ClientType, [{active, true}]) of
        {ok, Socket} ->
            Term =
                {?CONFD_PROTO_WORKER,
                 Dx#confd_daemon_ctx.daemon_id,
                 SockId},
            case econfd_internal:term_write(Socket, Term) of
                ok ->
                    WorkerPid =
                        proc_lib:spawn_link(econfd_daemon, worker,
                                            ['transient', Dx]),
                    case
                        econfd_internal:controlling_process(Socket, WorkerPid)
                    of
                        ok ->
                            WorkerDx = Dx#confd_daemon_ctx{
                                         wint = SockId,
                                         worker = Socket,
                                         worker_pid = WorkerPid
                                        },
                            WorkerPid ! {new_dx, WorkerDx},
                            {Socket,
                             UserInfo#confd_user_info{
                               actx = Actx#confd_action_ctx{
                                        dx = WorkerDx
                                       }
                              }
                            };
