...
%% We cannot use the pass_fun module here since econfd needs to
%% function standalone (no dependencies).
-spec wrap_clearpass(#confd_user_info{}) -> #confd_user_info{}.
wrap_clearpass(#confd_user_info{clearpass = CP} = Uinfo)
  when is_binary(CP) ->
    Uinfo#confd_user_info{clearpass = fun() -> CP end};
wrap_clearpass(#confd_user_info{} = Uinfo) ->
    Uinfo.

-spec unwrap_clearpass(#confd_user_info{}) -> #confd_user_info{}.
unwrap_clearpass(#confd_user_info{clearpass = PassFun} = Uinfo)
  when is_function(PassFun, 0) ->
    Uinfo#confd_user_info{clearpass = PassFun()};
unwrap_clearpass(#confd_user_info{} = Uinfo) ->
    Uinfo.
