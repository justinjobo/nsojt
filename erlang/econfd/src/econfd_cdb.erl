...
%% cdb compaction API
-export([initiate_journal_compaction/1,
...
-type compaction_dbfile() :: ?EM_COMPACTION_A_CDB |
                             ?EM_COMPACTION_O_CDB |
                             ?EM_COMPACTION_S_CDB.
%% CDB files used for compaction. CDB file can be either<ul>
%% <li> 1 = A.cdb </li>
%% <li> 2 = O.cdb </li>
%% <li> 3 = S.cdb </li></ul>
...
new_session(Socket, DbType, Flags) ->
    ...
    Opts = [useikp] ++
        ...
        if (Flags band ?CDB_READ_COMMITTED) =/= 0    -> [read_committed];
           true                                      -> []
        end,
...
%% @doc Initiates a journal compaction on all CDB files.
-spec initiate_journal_compaction(Socket) -> Result when
      Socket :: econfd:socket(),
      Result :: 'ok'.
initiate_journal_compaction(Socket) ->
    request(#cdb_session{socket = Socket}, ?OP_INITIATE_COMPACTION).

%% @doc Initiates a journal compaction on Dbfile.
-spec initiate_journal_dbfile_compaction(Socket, Dbfile) -> Result when
      Socket :: econfd:socket(),
      Dbfile :: compaction_dbfile(),
      Result :: 'ok' | {'error', econfd:error_reason()}.
initiate_journal_dbfile_compaction(Socket, Dbfile) ->
    request(#cdb_session{socket = Socket},
            ?OP_INITIATE_DBFILE_COMPACTION,
            Dbfile).

%% @doc Retrieves compaction info on Dbfile.
-spec get_compaction_info(Socket, Dbfile) -> Result when
      Socket :: econfd:socket(),
      Dbfile :: compaction_dbfile(),
      Result :: {'ok', Info} | {'error', econfd:error_reason()},
      Info :: compaction_info().
get_compaction_info(Socket, Dbfile) ->
    CDB = #cdb_session{socket = Socket},
    request(CDB, ?OP_GET_COMPACTION_INFO, Dbfile).
