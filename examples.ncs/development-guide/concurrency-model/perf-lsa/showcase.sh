...
if [ $USECQ == "True" ]; then
    while : ; do
        arr1=($(echo "show devices device lower-nso-1 live-status devices commit-queue queue-item | icount" | ncs_cli -C -u admin))
        res1=${arr1[1]}
        arr2=($(echo "show devices device lower-nso-2 live-status devices commit-queue queue-item | icount" | ncs_cli -C -u admin))
        res2=${arr2[1]}
    ...
    arr=($(echo "show devices device lower-nso-1 live-status devices commit-queue completed queue-item failed | icount" | ncs_cli -C -u admin))
    res=${arr[1]}
    if [ ! "$res" == "0" ]; then
    ...
    arr=($(echo "show devices device lower-nso-2 live-status devices commit-queue completed queue-item failed | icount" | ncs_cli -C -u admin))
    res=${arr[1]}
    if [ ! "$res" == "0" ]; then
