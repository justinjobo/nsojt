...
printf "\n"
RI=$(NCS_IPC_PORT=$NODE2 ncs_cmd -c 'mrtrans; maapi_get "/high-availability/settings/reconnect-interval"')
RA=$(NCS_IPC_PORT=$NODE2 ncs_cmd -c 'mrtrans; maapi_get "/high-availability/settings/reconnect-attempts"')
ID=$((RI*RA))
while [[ $(NCS_IPC_PORT=$NODE2 ncs_cmd -o -c 'mrtrans; maapi_get "/high-availability/status/mode"') != "primary" ]]; do
    printf "${RED}#### Waiting for node 2 to fail reconnect to node 1 and assume primary role...\n${NC}"
    if [[ $ID > 0 ]]; then
        let ID--
    fi
    sleep 1
...
printf "\n\n"
while [[ $(NCS_IPC_PORT=$NODE2 ncs_cmd -o -c 'mrtrans; maapi_get "/high-availability/status/mode"') != "secondary" ]]; do
