...
public class l3vpnRFS {
    ...
    for (NavuContainer endpoint : endpoints.elements()) {
      try {
        ...
        if (CEonVPE.contains(ceName)) {
          ...
          if (!addToTopology(context, service, ncsRoot, conn,
          ...
          // Add VPE-P3 topology
          if (!addToTopology(context, service, ncsRoot, null,
  ...
  private boolean addToTopology(ServiceContext context, NavuNode service,
                                NavuNode root,
    ...
    throws Exception
  {
    ...
    IPAddressAllocator.subnetRequest(service, RedeployType.DEFAULT,
                                     ipPoolName, "admin", prefixLength,
                                     connectionName, false, true);

    IdAllocator.idRequest(context, service, RedeployType.DEFAULT,
                          Settings.idPoolName, "admin",
                          connectionName, true, -1L, true);
    ...
    try {
      ...
      if (null == connection) {
        ...
        ConfUInt32 vlanID =
            IdAllocator.idRead(service.context(),
                               Settings.idPoolName,
                               connectionName);
      ...
      ConfIPPrefix net =
          IPAddressAllocator.subnetRead(service.context(),
                                        ipPoolName,
                                        connectionName);
