...
int main(int argc, char *argv[])
{
    ...
    // Prevent a buffer-overflow exploit of the ip char array
    if (ssh_connection != NULL && strlen(ssh_connection) > 1024) {
        fprintf(stderr,
                "Failed while reading env variable SSH_CONNECTION\n");
        exit(1);
    }
    long ip_size = 1024;
    char ip[ip_size];
    if (ssh_connection) {
        ...
        if (end != NULL) {
            ...
            if (end != NULL) {
                ...
                n = end - p;
                /* address/port */
                snprintf(&ip[iplen], ip_size-iplen, "/%.*s", n, p);
            }
        }
        else {
