...
EXT_API_FUN(_cdb_initiate_journal_dbfile_compaction,
            EXT_API_FUN_CDB_INITIATE_JOURNAL_DBFILE_COMPACTION)
{
    static char *kwlist[] = {
        "sock",
        "dbfile",
    ...
    CHECK_CONFD_ERR(cdb_initiate_journal_dbfile_compaction(s, dbfile));
    Py_RETURN_NONE;
}

/* ------------------------------------------------------------------------- */

EXT_API_FUN(_cdb_get_compaction_info,
            EXT_API_FUN_CDB_GET_COMPACTION_COMPACTION)
{
    static char *kwlist[] = {
        "sock",
        "dbfile",
    ...
    CHECK_CONFD_ERR(cdb_get_compaction_info(s, dbfile, &info));
    return Py_BuildValue(
        "{sKsKsKsi}",
        "fsize_previous", info.fsize_previous,
        "fsize_current", info.fsize_current,
                                  ...
                                  void *statep)
{
    ...
    if (state->opaque && state->opaque != Py_None) {
...
EXT_API_FUN(_cdb_sub_abort_trans, EXT_API_FUN_CDB_SUB_ABORT_TRANS)
{
    ...
    int s;
    u_int32_t apptag_ns, apptag_tag;
    ...
    CHECK_CONFD_ERR(cdb_sub_abort_trans(s, code, apptag_ns, apptag_tag, "%s",
                                        reason));
    Py_RETURN_NONE;
}
...
static PyMethodDef _cdb_Methods[] = {
    ...
    PYMOD_ENTRY(initiate_journal_dbfile_compaction),
    PYMOD_ENTRY(get_compaction_info),
