/*
 * Copyright 2013-2014 Tail-F Systems AB
 * Copyright 2014-2023 Cisco Systems, Inc. and/or its affiliates.
 * All rights reserved.
 *
...
static int attrs_arg(PyObject *attrs, py_prop_list_t *pl)
{
    if (attrs == Py_None) {
        return 1;
    }
    return confd_py_alloc_py_prop_list(attrs, pl, "attrs");
}

static int links_arg(PyObject *links, py_proglink_list_t *pll)
{
    if (links == Py_None) {
        return 1;
    }
    return confd_py_alloc_py_proglink_list(links, pll, "links");
}

static int span_arg(PyObject *obj, confd_progress_span *span)
{
    if (PyDict_Check(obj)) {
        if ((obj = PyDict_GetItemString(obj, "span_id")) == NULL) {
            PyErr_Format(PyExc_KeyError, "span_id");
            return 0;
        }
    }
    if (!PyString_Check(obj)) {
        PyErr_Format(PyExc_TypeError, "span must be a dict or string");
        return 0;
    }
    py_string_strlcpy(span->span_id, obj, sizeof(span->span_id));
    return 1;
}

static PyObject *progdict(confd_progress_span *span)
{
    PyObject *d;
    ...
    if (span->span_id[0] != '\0') {
        PyObject *v = PyUnicode_FromString(span->span_id);
        PyDict_SetItemString(d, "span_id", v);
    }
    if (span->trace_id[0] != '\0') {
        PyObject *v = PyUnicode_FromString(span->trace_id);
        PyDict_SetItemString(d, "trace_id", v);
    }
    return d;
}

EXT_API_FUN(_maapi_start_progress_span,
            EXT_API_FUN_MAAPI_PROGRESS_SPAN)
{
    static char *kwlist[] = {
        "sock",
        "msg",
    ...
    int sock;
    confd_progress_span res;
    const char *msg;
    enum confd_progress_verbosity verbosity =
        CONFD_VERBOSITY_NORMAL;
    py_prop_list_t attrs = {0};
    py_proglink_list_t links = {0};
    char *path = NULL;

    if (!PyArg_ParseTupleAndKeywords(args, kwds,
            "O&s|iO&O&O&", kwlist,
            sock_arg, &sock, &msg, /*optional:*/ &verbosity,
            attrs_arg, &attrs, links_arg, &links, path_arg, &path)) {
        return NULL;
    }
    CHECK_CONFD_ERR_EXEC(
        maapi_start_progress_span(sock,
            &res, msg, verbosity,
            attrs.list, attrs.size, links.list, links.size,
            path == NULL ? "/" : path),
        {
            confd_py_free_py_prop_list(&attrs);
            confd_py_free_py_proglink_list(&links);
            free(path);
        });
    return progdict(&res);
}

EXT_API_FUN(_maapi_start_progress_span_th,
            EXT_API_FUN_MAAPI_PROGRESS_SPAN_TH)
{
    static char *kwlist[] = {
        "sock",
        "thandle",
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwds,
            "O&is|iO&O&O&", kwlist,
            sock_arg, &sock, &thandle, &msg, /*optional:*/ &verbosity,
            attrs_arg, &attrs, links_arg, &links, path_arg, &path)) {
        return NULL;
    }
    CHECK_CONFD_ERR_EXEC(
        maapi_start_progress_span_th(
            sock, thandle, &res, msg, verbosity,
            attrs.list, attrs.size, links.list, links.size,
            path == NULL ? "/" : path),
        {
            confd_py_free_py_prop_list(&attrs);
...
EXT_API_FUN(_maapi_progress_info,
            EXT_API_FUN_MAAPI_PROGRESS_INFO)
{
    static char *kwlist[] = {
        "sock",
        "msg",
...
EXT_API_FUN(_maapi_progress_info_th,
            EXT_API_FUN_MAAPI_PROGRESS_INFO_TH)
{
    static char *kwlist[] = {
        "sock",
        "thandle",
...
EXT_API_FUN(_maapi_end_progress_span,
            EXT_API_FUN_MAAPI_END_PROGRESS_SPAN)
{
    static char *kwlist[] = {
        "sock",
        "span",
    ...
    confd_progress_span span = {0};

    if (!PyArg_ParseTupleAndKeywords(args, kwds,
            "O&O&|z", kwlist,
            sock_arg, &sock, span_arg, &span, &annotation)) {
        return NULL;
    }
    CHECK_CONFD_ERR(maapi_end_progress_span(
        sock, &span, annotation));
    Py_RETURN_NONE;
}
...
static PyMethodDef _maapi_Methods[] = {
    ...
    PYMOD_ENTRY(report_progress),
    PYMOD_ENTRY(start_progress_span),
    PYMOD_ENTRY(start_progress_span_th),
    PYMOD_ENTRY(progress_info),
    PYMOD_ENTRY(progress_info_th),
    PYMOD_ENTRY(end_progress_span),
