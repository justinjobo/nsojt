...
typedef struct {
    int size;
    struct confd_progress_link *list;
} py_proglink_list_t;
...
int py_string_strlcpy(char *dest, PyObject *src, int size);

int confd_py_alloc_py_prop_list(PyObject *o, py_prop_list_t *pl,
                                const char *argname);
void confd_py_free_py_prop_list(py_prop_list_t *pl);

int confd_py_alloc_py_proglink_list(PyObject *arg, py_proglink_list_t *pll,
                                    const char *argname);
