...
struct cdb_compaction_info {
    uint64_t fsize_previous; /* size after previous compaction in bytes */
    uint64_t fsize_current; /* current size in bytes */
    uint64_t last_time; /* last compaction time */
    unsigned int ntrans; /* number of transactions since last compaction */
};
...
#define CDB_SUB_FLAG_HA_IS_SECONDARY (1 << 4)

/* Backward compatibility, will be removed */
#define CDB_SUB_FLAG_HA_IS_SLAVE CDB_SUB_FLAG_HA_IS_SECONDARY
...
struct cdb_txid {
    ...
    u_int32_t       s2;
    u_int32_t       s3;
    char primary[MAXHOSTLEN];  /* NUL terminated string */
    /* Backward compatibility, will be removed */
    char *master;  /* points to primary */
};
...
extern int cdb_initiate_journal_dbfile_compaction(int sock,
                                                  enum cdb_dbfile_type dbfile);
extern int cdb_get_compaction_info(int sock,
                                   enum cdb_dbfile_type dbfile,
