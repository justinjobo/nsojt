/*
 * Copyright 2005-2014 Tail-F Systems AB
 * Copyright 2014, 2017-2023 Cisco Systems, Inc. and/or its affiliates.
 * All rights reserved.
 */
...
enum confd_compaction_dbfile {
    CONFD_COMPACTION_A_CDB = 1,  /* configuration datastore */
    CONFD_COMPACTION_O_CDB = 2,  /* operational datastore */
    CONFD_COMPACTION_S_CDB = 3   /* snapshot datastore */
};

enum confd_compaction_type {
    CONFD_COMPACTION_AUTOMATIC = 1,
    CONFD_COMPACTION_MANUAL    = 2
};
...
/* event delivered from CONFD_NOTIF_COMPACTION flag */
struct confd_compaction_notification {
    enum confd_compaction_dbfile dbfile;
    enum confd_compaction_type type;
    uint64_t fsize_start; /* size at start of compaction, bytes */
    uint64_t fsize_end;   /* size at end of compaction, bytes */
    uint64_t fsize_last;  /* size at end of last compaction, bytes */
    uint64_t time_start;  /* microseconds since epoch */
    uint64_t duration;    /* microseconds since epoch */
    unsigned int ntrans;  /* n transactions since last compaction */
};
...
struct confd_progress_notification {
    ...
    char trace_id[MAX_TRACE_ID_LEN];      /* unique trace id                  */
    char span_id[MAX_SPAN_ID_LEN];        /* span id                          */
    char parent_span_id[MAX_SPAN_ID_LEN]; /* parent span id                   */
    int usid;                             /* user session id (0 means n/a)    */
    int tid;                              /* transaction id (0 means n/a)     */
    ...
    struct confd_progress_attribute *attributes; /* attributes to the event   */
    int num_links;                        /* number of links                  */
    struct confd_progress_link *links;    /* links to other events            */
};

enum confd_progress_attribute_type {
    CONFD_PROGRESS_ATTRIBUTE_STRING = 1,
    CONFD_PROGRESS_ATTRIBUTE_NUMBER = 2
};

/* an attribute to the progress event */
struct confd_progress_attribute {
    char *name;
    enum confd_progress_attribute_type type;
    union {
        char *string;
...
/* This is a bitmask */
typedef int64_t confd_notification_type;

#define CONFD_NOTIF_AUDIT                  (1LL << 0)
#define CONFD_NOTIF_DAEMON                 (1LL << 1)
#define CONFD_NOTIF_TAKEOVER_SYSLOG        (1LL << 2)
#define CONFD_NOTIF_COMMIT_SIMPLE          (1LL << 3)
#define CONFD_NOTIF_COMMIT_DIFF            (1LL << 4)
#define CONFD_NOTIF_USER_SESSION           (1LL << 5)
#define CONFD_NOTIF_HA_INFO                (1LL << 6)
#define CONFD_NOTIF_SUBAGENT_INFO          (1LL << 7)
#define CONFD_NOTIF_COMMIT_FAILED          (1LL << 8)
#define CONFD_NOTIF_SNMPA                  (1LL << 9)
#define CONFD_NOTIF_FORWARD_INFO           (1LL << 10)
#define CONFD_NOTIF_NETCONF                (1LL << 11)
#define CONFD_NOTIF_DEVEL                  (1LL << 12)
#define CONFD_NOTIF_HEARTBEAT              (1LL << 13)
#define CONFD_NOTIF_CONFIRMED_COMMIT       (1LL << 14)
#define CONFD_NOTIF_UPGRADE_EVENT          (1LL << 15)
#define CONFD_NOTIF_COMMIT_PROGRESS        (1LL << 16)
#define CONFD_NOTIF_AUDIT_SYNC             (1LL << 17)
#define CONFD_NOTIF_HEALTH_CHECK           (1LL << 18)
#define CONFD_NOTIF_STREAM_EVENT           (1LL << 19)
#define CONFD_NOTIF_HA_INFO_SYNC           (1LL << 20)
#ifdef CONFD_C_PRODUCT_NCS
#define NCS_NOTIF_PACKAGE_RELOAD           (1LL << 21)
#define NCS_NOTIF_CQ_PROGRESS              (1LL << 22)
#endif /* CONFD_C_PRODUCT_NCS */
#define CONFD_NOTIF_REOPEN_LOGS            (1LL << 23)
#define CONFD_NOTIF_PROGRESS               (1LL << 24)
#ifdef CONFD_C_PRODUCT_NCS
#define NCS_NOTIF_CALL_HOME_INFO           (1LL << 25)
#endif /* CONFD_C_PRODUCT_NCS */
#define CONFD_NOTIF_JSONRPC                (1LL << 26)
#define CONFD_NOTIF_WEBUI                  (1LL << 27)
#ifdef CONFD_C_PRODUCT_NCS
#define NCS_NOTIF_AUDIT_NETWORK            (1LL << 28)
#define NCS_NOTIF_AUDIT_NETWORK_SYNC       (1LL << 29)
#endif /* CONFD_C_PRODUCT_NCS */
#define CONFD_NOTIF_COMPACTION             (1LL << 30)
...
struct confd_notification {
    ...
    union {
        ...
        struct confd_compaction_notification compaction;
#ifdef CONFD_C_PRODUCT_NCS
...
extern int confd_notifications_connect2(
    ...
    int srv_sz, confd_notification_type mask,
