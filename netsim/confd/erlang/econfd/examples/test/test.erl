...
collect_refs(M, TH, IKP) ->
    Cursor = econfd_maapi:init_cursor(M, TH, IKP),
    lists:reverse(
      collect_refs_loop(M, TH, econfd_maapi:get_next(Cursor), [])).

collect_refs_loop(_M, _TH, done, Ack) ->
    Ack;
collect_refs_loop(M, TH, {ok, Key, C}, Ack) ->
    collect_refs_loop(M, TH, econfd_maapi:get_next(C), [Key|Ack]).
...
loop_objs(M, TH, {ok, Key, C2}, Ack) ->
    ...
    NAck = case Key of
               ...
               _ ->
                   ?line {ok, Date} = Ret,
                   %% Get all leaf-list elements in one go
                   ?line LeafList =
                       collect_leaf_list_elements(M, TH, [refs, Key| T]),
                   ?line Refs = case LeafList of
                                    {ok, []}
                                      when Key == {1001} ; Key == {1011} ->
                                        [];
...
collect_leaf_list_elements(M, TH, IKeyPath) ->
    Cursor = econfd_maapi:init_cursor(M, TH, IKeyPath),
    GN = econfd_maapi:get_next(Cursor),
    case collect_leaf_list_elements(GN) of
        Res -> {ok, Res}
    end.

collect_leaf_list_elements(done) -> [];
collect_leaf_list_elements({ok, {E}, Cursor}) ->
    [E | collect_leaf_list_elements(econfd_maapi:get_next(Cursor))].
...
cdb_write_sub() ->
          ...
          fun() ->
                  try
                      ...
                      F = fun(Update) ->
                                  ...
                                  ?line {ok, Aes256Crypt} =
                                      econfd_cdb:get_elem(
                                        CDB, [aes256, encrypted, [NS|cdbw]]),
                                  ?line {ok, Des3} = econfd:decrypt(Des3Crypt),
                                  ?line {ok, Aes} = econfd:decrypt(AesCrypt),
                                  ?line {ok,Aes256}=econfd:decrypt(Aes256Crypt),

                                  Self ! {self(), Des3, Aes, Aes256},
                                  ?line ok = econfd_cdb:close(CDB),
                                  close
                          end,
    ...
    ?line {ok, CDB15} = econfd_cdb:new_session(S, ?CDB_OPERATIONAL, 0),
    ?line ok = econfd_cdb:set_elem2(CDB15, <<"baz">>,
                                    [aes256, encrypted, [NS|cdbw]]),
    ?line {ok, S} = econfd_cdb:end_session(CDB15),
    ...
    ?line {<<"foo">>, <<"bar">>, <<"baz">>} = receive
                                       {Pid, V1, V2, V3} ->
                                           {V1, V2, V3}
                                   after 20000 ->
                                           timeout
                                   end,
    io:format("foo bar baz received!!\n",[]),
        ...
        proc_lib:spawn_link(
          fun() ->
                  try
                      ...
                      ?line put(notif_number, 0),
                      F = fun(Update) ->
                                  io:format("Received oper update: ~p~n",
                                            [Update]),
                                  Self ! {self(), triggered},
                                  case get(notif_number) of
                                      2 ->
                                          %% Close sub when received 3 notifs.
                                          close;
                                      N ->
                                          put(notif_number, N+1),
                                          ?CDB_DONE_OPERATIONAL
                                  end
                          end,
                      ?line ok = econfd_cdb:wait(Sub, 20000, F),
                      ?line erase(notif_number)
                  catch
                      _:Err:Stacktrace ->
                          erep(Err, Stacktrace),
                          exit(error)
...
cdb() ->
    ...
    ?line {ok, 3} = econfd_cdb:next_index(CDB,
                                          [{<<"smtp">>},server,[NS|servers]]),
    %% ENG-24457,
    %% test econfd_cdb:index() return index of key
    ?line {ok, 2} = econfd_cdb:index(CDB, [{<<"smtp">>}, server, [NS|servers]]),

    %% ENG-24460, test bad keypath on list/leaf-list api
    ?line {error,
           {?CONFD_ERR_UNAVAILABLE,
            <<"num_instances() not allowed on given node type">>}} =
        econfd_cdb:num_instances(CDB, [name,{<<"smtp">>},server,[NS|servers]]),
    ?line {error,
           {?CONFD_ERR_UNAVAILABLE,
            <<"next_index() not allowed on given node type">>}} =
        econfd_cdb:next_index(CDB, [name,{<<"smtp">>},server,[NS|servers]]),
    ?line {error,
           {?CONFD_ERR_UNAVAILABLE,
            <<"index() not allowed on given node type">>}} =
        econfd_cdb:index(CDB, [name,{<<"smtp">>},server,[NS|servers]]),
    ...
    F = fun(Update) ->
                ...
                Iter = fun(IKP, Op, OldValue, Value, State) ->
                               io:format("CDB_ITER: IKP = ~999p, Op = ~p, "
                                         "Value = ~999p -> ~999p~n",
...
maapi() ->
    ...
    ?line ok = econfd_maapi:start_user_session(M, <<"admin">>, <<"maapi">>,
                                               [<<"admin">>], {127,0,0,1},
                                                ?CONFD_PROTO_TCP),
    {ok, USId} = econfd_maapi:get_my_user_session_id(M),
    {ok, {[<<"admin">>]}} = econfd_maapi:get_authorization_info(M, USId),
...
iter_validate(IKP, Op, OldValue, Value, State) ->
    io:format("ITER_validate: IKP = ~999p, Op = ~p, Value = ~999p~n",
              [IKP, Op, Value]),
...
validate2(Tctx, IKP, ?CONFD_ENUM_VALUE( NewVal)) when is_integer(NewVal) ->
    io:format("Validate2 ~999p --> ~p~n", [IKP, NewVal]),
...
validate(Tctx, IKP, NewVal) ->
    io:format("Validate ~999p --> ~p~n", [IKP, NewVal]),
    M = (Tctx#confd_trans_ctx.dx)#confd_daemon_ctx.d_opaque,
...
action_maapi_th() ->
    ...
    %% Set X=100 so that we can test that the action callback
    %% can make use of the Transaction Handle to attach to
    %% a MAAPI socket and read the value.
    X_IKP = [x, [NS|top]],
    ?line ok = econfd_maapi:set_elem(M, TH, X_IKP, ?CONFD_INT32(100)),

    IKP = [math, {?CONFD_BUF(<<"fred">>)}, host, [NS|top]],
    P1 =  [{add, start}, {op1, 2}, {op2, 3}, {add, stop}],
    %% If set, X will be added , i.e 2+3+X = 105
    ?line {ok, [{[NS|result], 105}]} = econfd_maapi:request_action_th(M, TH, P1, IKP),
    P2 =  [{del, start}, {op1, 65}, {op2, 23}, {del, stop}],
    ?line {ok, [{[NS|result], 42}]} = econfd_maapi:request_action_th(M, TH, P2, IKP),
    PErr =  [{del, start}, {op1, 65}, {del, stop}],
    ?line {error, {?CONFD_ERR_NOTSET, Bin}} =
        econfd_maapi:request_action_th(M, TH, PErr, IKP),
    ?line true = (string:str(binary_to_list(Bin), "/op2 is not provided") /= 0),
...
%% NOTE: This callback will be invoked by the test code
%% both with a proper Transaction Handle (TH) and without
%% no TH. To test that we can make use of the provided
%% TH we will read the leaf 'X' from the transaction and,
%% if set, make use of it in the addition operation.
math_cs(Uinfo, Name, IKeyPath, Params) ->
    [Ns|math] = Name,
    #confd_user_info{username = <<"admin">>,  % assertion
                     actx = Actx} = Uinfo,
    #confd_action_ctx{thandle = TH} = Actx,

    X = read_x_value(Uinfo, TH, Ns),
    put(x, X),
...
read_x_value(_Uinfo, -1, _NS) ->
    0;
read_x_value(Uinfo, TH, NS) ->
    Tctx = #confd_trans_ctx{uinfo = Uinfo,
                            thandle = TH},

    ?line {ok, M} = econfd_maapi:connect({127,0,0,1}, ?CONFD_PORT),
    ?line ok = econfd_maapi:attach(M, 0, Tctx),

    X_IKP = [x, [NS|top]],
    ?line {ok,X} = econfd_maapi:get_elem(M, TH, X_IKP),
...
iterate_crash() ->
    ...
    ?line ok = try
                   ...
                   error:badarith:Stacktrace ->
                       case
                           lists:keyfind(iter_fun, 2, Stacktrace)
                       of
                           false ->
                               bad1;
                           _ ->
...
make_ets(Name) ->
    ...
    spawn(fun() ->
                  ...
                  ets:new(Name, [public, {keypos, 2}, named_table]),
                  Self ! {make_ets_done, Name},
