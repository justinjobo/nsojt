...
-define(MAAPI_NCS_APPLY_TRANS_PARAMS,  150).
-define(MAAPI_NCS_GET_TRANS_PARAMS,    151).
...
-define(MAAPI_LOAD_MNS_MAPS,         264).
-define(MAAPI_GET_MOUNT_ID,          265).
-define(MAAPI_XPATH2HKP_TH,          266).
-define(MAAPI_REPORT_PROGRESS,       267).
-define(MAAPI_REPORT_PROGRESS_TH,    268).
-define(MAAPI_DESTROY_CURSOR,        269).
-define(MAAPI_GET_ROLLBACK_ID,       270).
-define(MAAPI_PROGRESS_SPAN,         271).
-define(MAAPI_PROGRESS_SPAN_TH,      272).
...
%% NCS specific
-define(MAAPI_NCS_COMMIT_SAVE_TRANS,       360).% Deprecated on 7.1
-define(MAAPI_NCS_APPLY_DIFF_SET,          361).% Deprecated on 7.1
-define(MAAPI_NCS_TOOGLE_FASTMAP,          362).
-define(MAAPI_NCS_FUN_CALL,                363).
-define(MAAPI_NCS_SHARED_CREATE,           364).
-define(MAAPI_NCS_MOVE_PRIVATE_DATA,       365).% Deprecated on 8.1
...
-define(MAAPI_NCS_UPDATE_PLAN_HISTORY,     372).% Deprecated on 7.1
-define(MAAPI_NCS_COMMIT_SAVE_NO_DIFFSET,  373).% Deprecated on 7.1
...
%% NETCONF interaction
-define(MAAPI_NETCONF_SSH_CALL_HOME,        450).
%% The function below will be used when we implement ietf-netconf-server,
%% and clients are referred to by endpoint name.  The idea is that this
%% function takes a single parameter, the endpoint name.
%% define(MAAPI_NETCONF_CALL_HOME_BY_NAME, 451).
-define(MAAPI_NETCONF_SSH_CALL_HOME_OPAQUE, 452).
...
-define(MAAPI_CMD_KEEP_PIPE,           (1 bsl 3)).

%% Bit Flags for saving/loading config

%% TEMP for trac #15190
%% When save config, only work on node that have read_write access.
...
-define(MAAPI_LISTENER_NETCONF_CALL_HOME, (1 bsl 5)).

%%- COMMON END

%% Defined as enumerations in JAVA and C.
%% FIXME: Move into the common ones.
-define(MAAPI_PROGRESS_START, 1).
-define(MAAPI_PROGRESS_STOP,  2).
-define(MAAPI_PROGRESS_INFO,  3).
