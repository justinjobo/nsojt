...
get_logsym(?LOGSYM_FXS_MISMATCH) -> {"FXS_MISMATCH","A secondary connected to a primary where the fxs files are different"};
get_logsym(?LOGSYM_TOKEN_MISMATCH) -> {"TOKEN_MISMATCH","A secondary connected to a primary with a bad auth token"};
get_logsym(?LOGSYM_HA_SECONDARY_KILLED) -> {"HA_SECONDARY_KILLED","A secondary node didn't produce its ticks"};
get_logsym(?LOGSYM_HA_DUPLICATE_NODEID) -> {"HA_DUPLICATE_NODEID","A secondary arrived with a node id which already exists"};
get_logsym(?LOGSYM_HA_FAILED_CONNECT) -> {"HA_FAILED_CONNECT","An attempted library become secondary call failed because the secondary couldn't connect to the primary"};
get_logsym(?LOGSYM_HA_BAD_VSN) -> {"HA_BAD_VSN","A secondary connected to a primary with an incompatible HA protocol version"};
...
get_logsym(?LOGSYM_SSH_HOST_KEY_UNAVAILABLE) -> {"SSH_HOST_KEY_UNAVAILABLE","No SSH host keys available."};
get_logsym(?LOGSYM_COMMIT_QUEUE_CORRUPT) -> {"COMMIT_QUEUE_CORRUPT","Failed to load commit queue. ConfD recovers by starting from an empty commit queue."};
get_logsym(?LOGSYM_NCS_PACKAGE_AUTH_SUCCESS) -> {"NCS_PACKAGE_AUTH_SUCCESS","A package authenticated user logged in."};
get_logsym(?LOGSYM_NCS_PACKAGE_AUTH_FAIL) -> {"NCS_PACKAGE_AUTH_FAIL","Package authentication failed."};
get_logsym(?LOGSYM_NCS_PACKAGE_AUTH_BAD_RET) -> {"NCS_PACKAGE_AUTH_BAD_RET","Package authentication program returned badly formatted data."};
get_logsym(?LOGSYM_JIT_ENABLED) -> {"JIT_ENABLED","Show if JIT is enabled."};
get_logsym(?LOGSYM_NIF_LOG) -> {"NIF_LOG","Log message from NIF code."};
get_logsym(?LOGSYM_MAAPI_WRITE_TO_SOCKET_FAIL) -> {"MAAPI_WRITE_TO_SOCKET_FAIL","maapi failed to write to a socket."};
get_logsym(?LOGSYM_NCS_PACKAGE_CHAL_2FA) -> {"NCS_PACKAGE_CHAL_2FA","Package authentication challenge sent to a user."};
get_logsym(?LOGSYM_NCS_PACKAGE_CHAL_FAIL) -> {"NCS_PACKAGE_CHAL_FAIL","Package authentication challenge failed."};
