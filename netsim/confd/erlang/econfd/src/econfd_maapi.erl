...
%%% @version {$Id$}
%%% @doc An Erlang interface equivalent to the MAAPI C-API
%%%
...
%% Keep this in sync with econfd.hrl
-type verbosity() :: ?CONFD_PROGRESS_NORMAL |
                          ?CONFD_PROGRESS_VERBOSE |
                          ?CONFD_PROGRESS_VERY_VERBOSE |
                          ?CONFD_PROGRESS_DEBUG.
%% The type is used in {@link start_span_th/7} and can be either<ul>
%% <li> 0 = CONFD_PROGRESS_NORMAL </li>
%% <li> 1 = CONFD_PROGRESS_VERBOSE </li>
%% <li> 2 = CONFD_PROGRESS_VERY_VERBOSE </li>
%% <li> 3 = CONFD_PROGRESS_DEBUG </li></ul>
...
%% @deprecated Consult the /rollback-files list instead.
...
%% @deprecated Use the action /rollback-files/apply-rollback-file instead.
...
%%
%% Does the same thing as request_action/3, but uses
%% the current namespace, the path position, and the user session
%% from the transaction indicated by the 'Tid' handle.
-spec request_action_th(Socket, Tid, Params, IKeypath) -> Result when
      Socket :: econfd:socket(),
      Tid :: integer(),
      Params :: [econfd:tagval()],
      IKeypath :: econfd:ikeypath(),
      Result :: 'ok' | {'ok', [econfd:tagval()]} | err().
request_action_th(Sock, Tid, Params, IKP) ->
    RIKP = reverse(IKP),   % NOTE, must be real reverse
    R = {{exml, Params}, RIKP},
    intcall(Sock, ?MAAPI_REQUEST_ACTION_TH, Tid, R).
...
-spec start_progress_span(Socket, Verbosity, Msg, SIKP, Attrs, Links) ->
      ...
      Links :: [{TraceId :: binary() | undefined,
          SpanId1 :: binary() | undefined}],
      Result :: {ok, {SpanId2 :: binary() | undefined,
          TraceId :: binary() | undefined}}.
start_progress_span(Sock, Verbosity, Msg, SIKP, Attrs, Links) ->
    Term = {?MAAPI_PROGRESS_START, Verbosity, Msg, reverse(SIKP), Attrs, Links},
    intcall(Sock, ?MAAPI_PROGRESS_SPAN, -1, Term).

-spec start_progress_span_th(Socket, Tid, Verbosity, Msg, SIKP, Attrs, Links) ->
          Result when
      Socket :: econfd:socket(),
      Tid :: integer(),
      Verbosity :: verbosity(),
      ...
      Result :: {ok, {SpanId2 :: binary() | undefined,
          TraceId :: binary() | undefined}}.
start_progress_span_th(Sock, Tid, Verbosity, Msg, SIKP, Attrs, Links) ->
    Term = {?MAAPI_PROGRESS_START, Verbosity, Msg, reverse(SIKP), Attrs, Links},
    intcall(Sock, ?MAAPI_PROGRESS_SPAN_TH, Tid, Term).

-spec progress_info(Socket, Verbosity, Msg, SIKP, Attrs, Links) ->
          ok when
      Socket :: econfd:socket(),
      Verbosity :: verbosity(),
      Msg :: iolist(),
...
progress_info(Sock, Verbosity, Msg, SIKP, Attrs, Links) ->
    Term = {?MAAPI_PROGRESS_INFO, Verbosity, Msg, reverse(SIKP), Attrs, Links},
    intcall(Sock, ?MAAPI_PROGRESS_SPAN, -1, Term),
    ok.
...
-spec progress_info_th(Socket, Tid, Verbosity, Msg, SIKP, Attrs, Links) ->
      ...
      Links :: [{TraceId :: binary() | undefined,
          SpanId :: binary() | undefined}].
progress_info_th(Sock, Tid, Verbosity, Msg, SIKP, Attrs, Links) ->
    Term = {?MAAPI_PROGRESS_INFO, Verbosity, Msg, reverse(SIKP), Attrs, Links},
    intcall(Sock, ?MAAPI_PROGRESS_SPAN_TH, Tid, Term),
    ok.

-spec end_progress_span(Socket, SpanId1, Annotation) -> Result when
      Socket :: econfd:socket(),
      SpanId1 :: binary(),
      Annotation :: iolist(),
      Result :: {ok, {SpanId2 :: binary() | undefined,
          TraceId :: binary() | undefined}}.
end_progress_span(Sock, SpanId, Annotation) ->
    Term = {?MAAPI_PROGRESS_STOP, SpanId, Annotation},
    intcall(Sock, ?MAAPI_PROGRESS_SPAN, -1, Term).
