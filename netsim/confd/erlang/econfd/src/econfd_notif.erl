...
handle_notif(Term) ->
    if element(1, Term) == ?CONFD_NOTIF_AUDIT ->
       ...
       element(1, Term) == ?CONFD_NOTIF_COMMIT_PROGRESS ->
            Event      = element(2, Term),
            Attributes = maps:from_list(element(14, Event)),
            Links      =
                lists:map(fun({TraceId, SpanId}) ->
                                  #econfd_progress_link{ trace_id = TraceId,
                                                         span_id  = SpanId
                                                       }
                          end, element(15, Event)),
            #econfd_notif_progress{
                             type            = element(1,  Event),
                             timestamp       = element(2,  Event),
                             duration        = element(3,  Event),
                             span_id         = element(4,  Event),
                             parent_span_id  = element(5,  Event),
                             trace_id        = element(6,  Event),
                             usid            = element(7,  Event),
                             tid             = element(8,  Event),
                             database        = element(9,  Event),
                             context         = element(10, Event),
                             subsystem       = element(11, Event),
                             msg             = element(12, Event),
                             annotation      = element(13, Event),
       ...
       element(1, Term) == ?CONFD_NOTIF_COMPACTION ->
            #econfd_notif_confd_compaction {
               dbfile = element(2, Term),
               type = element(3, Term),
               fsize_start = element(4, Term),
               fsize_end = element(5, Term),
               fsize_last = element(6, Term),
               time_start = element(7, Term),
               duration = element(8, Term),
               ntrans = element(9, Term)
              };
