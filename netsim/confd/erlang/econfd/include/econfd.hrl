...
-record(confd_user_info, {
          ...
          clearpass = <<>>,      %% binary() (empty if we don't have the pass)
                                 %% fun() -> binary() if stored in ETS table
...
-record(confd_daemon_ctx, {
          ...
          flags,               %% integer()
          %% Mapping between transaction and its worker
          transaction_handlers ::
            #{pid() => econfd_daemon:transaction_handler(),
              econfd_daemon:transaction_handler() => pid()}
         }).
...
-define(CDB_ITER_WANT_SUPPRESS_OPER_DEFAULTS,       (1 bsl 11)).
...
-define(NCS_NOTIF_AUDIT_NETWORK,      (1 bsl 28)).
-define(NCS_NOTIF_AUDIT_NETWORK_SYNC, (1 bsl 29)).
-define(CONFD_NOTIF_COMPACTION,       (1 bsl 30)).
...
%% compaction notificiation cdb files
-define(EM_COMPACTION_A_CDB, 1).
-define(EM_COMPACTION_O_CDB, 2).
-define(EM_COMPACTION_S_CDB, 3).

%% compaction notification types
-define(EM_COMPACTION_AUTOMATIC, 1).
-define(EM_COMPACTION_MANUAL,    2).
...
-define(CONFD_HA_STATE_SECONDARY,       2).
-define(CONFD_HA_STATE_PRIMARY,         3).
-define(CONFD_HA_STATE_SECONDARY_RELAY, 4).
...
-define(CONFD_HA_INFO_NOPRIMARY,           1).  % we have no primary
-define(CONFD_HA_INFO_SECONDARY_DIED,         2).  % a secondary disappeared
-define(CONFD_HA_INFO_SECONDARY_ARRIVED,      3).  % a secondary arrived to us
-define(CONFD_HA_INFO_SECONDARY_INITIALIZED,  4).  % CDB is initialized
-define(CONFD_HA_INFO_IS_PRIMARY,          5).  % we are now primary
-define(CONFD_HA_INFO_IS_NONE,            6).  % we are now none
% result of async besecondary():
-define(CONFD_HA_INFO_BESECONDARY_RESULT,     7).
...
-define(CONFD_HA_INFO_BESLAVE_RESULT,     7).  % result of async besecondary()
%% Deprecated end
...
-record(econfd_notif_ha, {
          ...
          data    %% depending on type, data is either of
                  %%
                  %% if ?CONFD_HA_INFO_NOPRIMARY  ?CONFD_ERR_HA_CLOSED |
                  %%                             ?CONFD_ERR_HA_NOTICK
                  %% if ?CONFD_HA_INFO_SECONDARY_DIED DeadSecondary::ha_node()
                  %% if ?CONFD_HA_INFO_SECONDARY_ARRIVED NewSecondary::ha_node()
                  %% if ?CONFD_HA_INFO_CDB_INITIALIZED initByCdbCopy::integer()
                  %% if ?CONFD_HA_INFO_BESECONDARY_RESULT 0 | ?CONFD_ERR_HA_XXX
         }).
...
-record(econfd_progress_link, {
          trace_id, %% binary() Unique trace id
...
-record(econfd_notif_progress, {
          ...
          attributes,      %% map()     Attributes to the event
          links            %% [#econfd_progress_link{}]
         }).
...
-record(econfd_notif_ncs_audit_network, {
          usid,      %% integer()
          tid,       %% integer()
          user,      %% binary()
          device,    %% binary()
...
-record(econfd_notif_confd_compaction, {
          dbfile      :: non_neg_integer(),
          type        :: non_neg_integer(),
          fsize_start :: non_neg_integer(),
          fsize_end   :: non_neg_integer(),
          fsize_last  :: non_neg_integer(),
          time_start  :: non_neg_integer(),
          duration    :: non_neg_integer(),
          ntrans      :: non_neg_integer()
         }).
...
-define(CONFD_PROGRESS_NORMAL, 0).
-define(CONFD_PROGRESS_VERBOSE, 1).
-define(CONFD_PROGRESS_VERY_VERBOSE, 2).
-define(CONFD_PROGRESS_DEBUG, 3).
