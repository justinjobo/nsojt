...
-record(identity, {
          ...
          moduleref :: yang:modrev(),
          bases = [] :: [#identity{}],
          if_feature = [] :: [{yang_if_feature:expr(),
                               Origin :: 'local',
                               yang:stmt()}],
...
-record(hooks, {
          ...
          %%   {#yctx{}, #sn{}}.
          %%
          %% post_parse_body_mk_children is called for each #sn. It
          %% has to run after the main body children has been created
          %% because the sn.type is needed. The HookF must have the
          %% same signature as post_mk_sn because these hooks will be
          %% moved there after the main mk_children is done.
          post_parse_body_mk_children = [] :: [yang:hookfun()
                            | {[Condition::term()] | yang:hookfun()}]
         }).
...
-record(yctx, {
          ...
          allow_invalid_config = false :: boolean(),
          %% Used during deviation type resolving
          target_name :: atom() | undefined,
          %% 'feature_del' is a list of stmts for unsupported features rendering
          %% the enclosing enumeration or bits invalid, but might be okay if
          %% they are inside a union.
          feature_del = [] :: [{yang:stmt(), yang:pos()}],
          %% true when successed tried loaded the main module of a submodule
          %% see yang_cursor:maybe_load_main_module/2
          involves_main_module = false :: boolean()
         }).

%% See yang_cursor:move()
-record(cursor, {
          ...
          last_skipped :: yang_cursor:skipped()
         }).
...
%% Used to discern a valid leafref cursor node
-define(is_valid_leafref_node(Kind, Type),
        ((Kind == 'leaf' orelse Kind == 'leaf-list') andalso
         is_tuple((Type#type.type_spec)#leafref_type_spec.path))).
