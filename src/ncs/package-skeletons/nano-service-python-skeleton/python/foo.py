...
class NanoServiceCallbacks(NanoService):
    ...
    def cb_nano_create(self, tctx, root, service, plan, component, state,
        ...
        if state == '%NAME%:vm-requested':
            # Create and initialize the %NAME% instance
            # Any '-' in %NAME% must be replaced with '_'
            vmi = root.%_NAME_%__vm_instance.create(service.name)
            vmi.type = 'csr-small'
...
class %CLASS%(ncs.application.Application):
    ...
    def setup(self):
        ...
        # and plan outline.
        self.register_nano_service('%NAME%-servicepoint',  # Service point
                                   'ncs:self',              # Component
                                   '%NAME%:vm-requested',       # State
