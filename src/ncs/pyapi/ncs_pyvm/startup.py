    ...
    try:
        ...
        for opt, arg in opts:
            if opt in ("-h", "--help"):
                ...
                log_file = arg
            elif opt in ("-s", "--self-assign-warning"):
                self_assign_warning = arg
            elif opt in ("-i", "--id"):
                vmid = arg

        return (vmid, log_file, log_level, self_assign_warning)
...
class Main(object):
    ...
    def __init__(self, vmid, self_assign_warning):
        self.vmid = vmid
        self.self_assign_warning = self_assign_warning
        self.exit = False
    ...
    def dispatch(self, msg):
        try:
            if msg['msg'] == 'echo':
            ...
            elif msg['msg'] == 'set_self_assign_warning':
                dbg('GOT SET_SELF_ASSIGN_WARNING')
                self.set_self_assign_warning(msg)
                ...
                warning('GOT UNKNOWN MESSAGE: ' + msg['msg'])
    ...
    def set_self_assign_warning(self, msg):
        if self.self_assign_warning == msg['warning']:
            info('Keeping self-assign-warning {} on {}'.format(
                self.self_assign_warning, self.vmid))
        else:
            self.self_assign_warning = msg['warning']
            info('Setting self-assign-warning {} on {}'.format(
                self.self_assign_warning, self.vmid))
            ncs.util.set_self_assign_warning(self.self_assign_warning)
            for component in self._components:
                component.set_self_assign_warning(self.self_assign_warning)
...
def main():
    (vmid, log_file, log_level, self_assign_warning) = \
    ...
    if sys.version_info < (3, 7):
        ...
        if ncs.util.is_multiprocessing():
            # The fork start method should be considered unsafe as it can lead
            # to crashes on mac, see bpo-33725 and deadlocks on Linux when used
            # with threads, see https://github.com/python/cpython/issues/84559
            # spawn is default on macos in python 3.8 and 3.14 will change
            # default away from fork for all platforms.
        ...
        if not ncs.util.set_kill_child_on_parent_exit():
            if ncs.util.is_multiprocessing():
                ...
                ncs.util.set_callpoint_model('threading')
        info('Using callpoint-model: {}'.format(ncs.util.get_callpoint_model()))

        # Start the main loop in a separate thread
        main = Main(vmid, self_assign_warning)
        main.run()
