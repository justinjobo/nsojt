...
class Maapi(object):
    ...
    def start_progress_span(self, msg, verbosity=_tm.VERBOSITY_NORMAL,
            attrs=None, links=None, path=None):
        ...
        * trace span (ncs.progress.Span)
        """
        if links != None:
            links = progress.conv_links(links)
        d = _tm.maapi.start_progress_span(
            self.msock, msg, verbosity, attrs, links, path)
        return progress.Span(self.msock, **d)

    def progress_info(self, msg, verbosity=_tm.VERBOSITY_NORMAL,
            attrs=None, links=None, path=None):
        ...
        * path - keypath to an action/leaf/service/etc (str, optional)
        ...
        if links != None:
            links = progress.conv_links(links)
        _tm.maapi.progress_info(self.msock, msg, verbosity, attrs, links, path)

    def end_progress_span(self, *args):
...
class Transaction(object):
    ...
    def start_progress_span(self, msg, verbosity=_tm.VERBOSITY_NORMAL,
        ...
        * trace span (ncs.progress.Span)
        ...
        if links != None:
            links = progress.conv_links(links)
        d = _tm.maapi.start_progress_span_th(
            self.maapi.msock, self.th, msg, verbosity, attrs, links, path)
        return progress.Span(self.maapi.msock, **d)
