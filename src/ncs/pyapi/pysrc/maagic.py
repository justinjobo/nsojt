...
def as_pyval(mobj, name_type=NODE_NAME_PY_SHORT,
    ...
    * enum_as_string -- return enumerator in str form (boolean)
    ...
    if hasattr(mobj, '_backend'):
        if mobj._backend is None and not is_oper(mobj):
            raise BackendError("Not a Transaction backend")
        trans = mobj._backend
    else:
        trans = None

    if not include_oper and is_oper(mobj):
        return None
    ...
    if isinstance(mobj, List):
        ...
        for mobj_value in mobj:
            value.append(as_pyval(
                mobj_value, name_type, include_oper, enum_as_string))
        return value

    if isinstance(mobj, (Container, Root, ActionParams)):
        ...
        for child in mobj._children.children.values():
            # This acts as a guard to the next invocation of hasattr, since
            # this will result in a call to get_value on a Choice node and
            # would raise an error for in-memory backends. We don't want to
            # output it anyways as seen below.
            if isinstance(child.obj, Choice) and trans is None:
                continue
            ...
            # a Choice may not be a Choice instance until we load it so we
            # can't remove the Choice check in this if statement
            if (not isinstance(child.obj, Choice)
                    and (include_oper or not is_oper(child.obj))):
                node = getattr(mobj, child.py_full_name)
                value[get_child_name(child)] = as_pyval(
                    node, name_type, include_oper, enum_as_string)
        return value
