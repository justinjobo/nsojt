...
class _StateManager(dp.StateManager):
    ...
    def setup(self, state, previous_state):
        ...
        validations = state['_app_validations'] = []
        ...
        if self._app_validations_cls:
            # transaction validation callback is required for
            # validation points, register one that supports threading.
            for (validation_pt, validation_cls,
                 init_args) in self._app_validations_cls:
                v = validation_cls(state, validation_pt, None, init_args)
                v.start()
                if previous_state is None:
                    self._app.add_running_thread(
                        validation_pt + ' (validation)')
    ...
    def teardown(self, state, finished):
        ...
        for v in state['_app_validations']:
            v.stop()
            if finished is True:
                self._app.del_running_thread(
                    v.validationpoint + ' (validation)')
    ...
    def register_validation(self, validation_pt, validation_cls,
                            init_args=None):
        self._app_validations_cls.append((validation_pt, validation_cls,
                                          init_args))
...
class _AppWorker(object):
    def __init__(self, model, cls, app_info, worker_id,
        ...
        self._app_validations_cls = app_validations_cls
        ...
        self._callpoints.extend(cp[0] for cp in app_validations_cls)

        # event used in wait() by the caller to ensure worker is up
        # before continuing.
        event_cls = getattr(self._model_mod(), 'Event')
        self._ready = event_cls()
        self._stop = event_cls()

    def start(self):
        name = '_AppWorker {}'.format(', '.join(self._callpoints))
        worker_cls = getattr(self._model_mod(), self._cls)
    ...
    def set_self_assign_warning(self, warning):
        # no need to set the warning type, same process so shared with
        # the parent.
        pass

    def _model_mod(self):
        if self._model == 'multiprocessing':
    ...
    def _run(self):
        ...
        self._state_mgr._app_validations_cls = self._app_validations_cls
    ...
    def _main(self):
        ...
        if self._app_validations_cls:
            self.dd.register_trans_validate_cb()
        ...
        while True:
            ...
            elif msg[0] == 'set_self_assign_warning':
                util.set_self_assign_warning(msg[1])
                self._put_msg(('self_assign_warning', True))
...
class _ProcessAppWorker(_AppWorker):
    ...
    def set_self_assign_warning(self, warning):
        self._ppipe.send(('set_self_assign_warning', warning))
        # wait for response from worker, to ensure self assign warning
        # has taken effect when this call release
        self._ppipe.recv()
...
class Application(ncs_pyvm.NcsPyVM):
    ...
    def __init__(self, *args, **kwds):
        ...
        # proccess
        if util.is_multiprocessing():
            import multiprocessing
            self._child_q = multiprocessing.Queue()
        else:
            import queue
            self._child_q = queue.Queue()
    ...
    def register_validation(self, validationpoint, validation_cls,
                            init_args=None):
        ...
        * init_args -- initial arguments (optional)
        """
        self._app_validations_cls.append((validationpoint, validation_cls,
                                          init_args))
    ...
    def set_self_assign_warning(self, warning):
        ...
        * warning -- warning type (alarm, log, off). (string)
        """
        for worker in self._workers:
            worker.set_self_assign_warning(warning)
    ...
    def _group_callpoints(self):
        if not util.is_multiprocessing():
            return [(self._app_actions_cls, self._app_services_cls,
                     self._app_nano_services_cls, self._app_validations_cls)]
        ...
        for app_validation in self._app_validations_cls:
            groups.append(([], [], [], [app_validation]))

        # create empty group that will take care of starting the
        # application functions if no other group exist.
        if not groups and self._app_info.app_funs:
            groups.append(([], [], [], []))
    ...
    def _start(self, groups):
        if len(groups) == 1:
            ...
            actions, services, nano_services, validations = groups[0]
            ...
            for group in groups:
                actions, services, nano_services, validations = group
...
class Service(object):
    ...
    def __setattr__(self, name, value):
        fun = util.get_setattr_fun(self, Service)
...
class NanoService(object):
    ...
    def __setattr__(self, name, value):
        fun = util.get_setattr_fun(self, NanoService)
        return fun(name, value)
        ...
        * compproplist - component properties (list(tuple(str, str)))
        ...
        def wrapper(self, tctx, kp, component, state, proplist,
            ...
            try:
                ...
                with util.with_setattr_check(skp):
                    pl = fn(self, tctx, root, service, planpath, component,
                            state, proplist, component_proplist)
                if pl is not None:
