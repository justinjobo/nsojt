...
static PyTypeObject *confdProgressLinkType = NULL;
...
/* ************************************************************************ */
/* struct confd_user_info* -> confd.UserInfo                                */
/* ************************************************************************ */
...
/* ************************************************************************ */
static const char *notif_type2str(confd_notification_type type)
{
    switch (type) {
        ...
        case CONFD_NOTIF_COMPACTION:
            return "CONFD_NOTIF_COMPACTION";
...
{
    confd_free_notification(&self->n);
    PY_TP_FREE(self);
}
...
static PyObject *confdNotification_str(confdNotification *self)
{
    PyObject *res = PyString_FromFormat(
            CONFD_PY_MODULE ".events.Notification(type=%s (%" PRIu64 "))",
...
/* ************************************************************************ */
/* struct confd_progress_link -> confd.ProgressLink                         */
/* ************************************************************************ */

static void confdProgressLink_dealloc(confdProgressLink *self)
{
    free(self->trace_id);
    free(self->span_id);
    /* Normal dealloc */
...
static PyObject *confdProgressLink_str(confdProgressLink *self)
{
    return PyString_FromFormat(
        CONFD_PY_MODULE ".ProgressLink(trace_id=%s, span_id=%s)",
        self->trace_id ? self->trace_id : "None",
        self->span_id ? self->span_id : "None");
}

static PyObject *confdProgressLink_repr(confdProgressLink *self)
{
    return confdProgressLink_str(self);
}

static int confdProgressLink_init(
    confdProgressLink *self, PyObject *args, PyObject *kwds)
{
    free(self->trace_id);
    free(self->span_id);
...
static PyObject *confdProgressLink_new(
    PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    confdProgressLink *self = (confdProgressLink *)PY_TP_ALLOC(type);
...
static PyMemberDef confdProgressLink_members[] = {
    {"trace_id", T_STRING, offsetof(confdProgressLink, trace_id),
     READONLY, confdProgressLink_attr_trace_id__doc__},
    {"span_id", T_STRING, offsetof(confdProgressLink, span_id),
     READONLY, confdProgressLink_attr_span_id__doc__},
    {NULL}  /* Sentinel */
};

/* Pickle the object */
static PyObject *
confdProgressLink___getstate__(confdProgressLink *self,
                               PyObject *Py_UNUSED(ignored)) {
    PyObject *ret = Py_BuildValue("{ssss}",
...
static PyObject *
confdProgressLink___setstate__(confdProgressLink *self, PyObject *state) {
    if (!PyDict_CheckExact(state)) {
        PyErr_SetString(PyExc_ValueError, "Pickled object is not a dict");
        return NULL;
    }

    PyObject *trace_id = PyDict_GetItemString(state, "trace_id");
    if (trace_id == NULL) {
        PyErr_SetString(PyExc_KeyError, "No \"trace_id\" in pickled dict");
        return NULL;
    }
    CONFD_PY_WITH_C_STR(trace_id, trace_id_c_str) {
        free(self->trace_id);
        self->trace_id = strdup(trace_id_c_str);
    }

    PyObject *span_id = PyDict_GetItemString(state, "span_id");
    if (span_id == NULL) {
        PyErr_SetString(PyExc_KeyError, "No \"span_id\" in pickled dict");
        return NULL;
    }
    CONFD_PY_WITH_C_STR(span_id, span_id_c_str) {
        free(self->span_id);
        self->span_id = strdup(span_id_c_str);
    }

    Py_RETURN_NONE;
}

static PyMethodDef confdProgressLink_methods[] = {
    {"__getstate__", (PyCFunction) confdProgressLink___getstate__, METH_NOARGS,
     "Pickle the confdProgressLink object"
    },
    {"__setstate__", (PyCFunction) confdProgressLink___setstate__, METH_O,
     "Un-pickle the confdProgressLink object"
    },
    {NULL}  /* Sentinel */
};

static int setup_type_confd_progresslink(void)
{
    PyType_Slot slots[] = {
        { .slot = Py_tp_new,         .pfunc = confdProgressLink_new },
        { .slot = Py_tp_init,        .pfunc = confdProgressLink_init },
        { .slot = Py_tp_dealloc,     .pfunc = confdProgressLink_dealloc },
        { .slot = Py_tp_members,     .pfunc = confdProgressLink_members },
        { .slot = Py_tp_methods,     .pfunc = confdProgressLink_methods },
        { .slot = Py_tp_doc,         .pfunc = (void*)confdProgressLink__doc__ },
        { .slot = Py_tp_repr,        .pfunc = confdProgressLink_repr },
        { .slot = Py_tp_str,         .pfunc = confdProgressLink_str },
        { .slot = 0, .pfunc = 0 }
    };

    PyType_Spec spec = {
        .name = CONFD_PY_MODULE ".ProgressLink",
        .basicsize = sizeof(confdProgressLink),
        .itemsize = 0,
        .flags = Py_TPFLAGS_DEFAULT|Py_TPFLAGS_BASETYPE,
        .slots = slots
    };

    if ((confdProgressLinkType = (PyTypeObject*)PyType_FromSpec(&spec)) == NULL)
        return -1;
    return 0;
}

PyObject *newConfdProgressLink(struct confd_progress_link *link)
{
    confdProgressLink *self =
        (confdProgressLink *)
            PyObject_New(confdProgressLink, confdProgressLinkType);

    if (self != NULL) {
        self->trace_id = strdup(link->trace_id);
        self->span_id = strdup(link->span_id);
    }

    return (PyObject*)self;
}

int isConfProgressLink(PyObject *o)
{
    return (o != NULL) && (o->ob_type == confdProgressLinkType);
}
...
void init_lib_types(PyObject *m)
{
    ...
    /* Setup of confd.ProgressLink */
    if (setup_type_confd_progresslink() < 0)
        goto error;
    PyModule_AddObject(m, "ProgressLink", (PyObject *)confdProgressLinkType);
