...
/*
    struct confd_progress_notification {
        ...
        struct attribute *attributes;        * attributes to the event
        int num_links;                       * number of links
        struct link *links;                  * links to other events
    };
*/
        ...
        PyObject *d, struct confd_progress_notification *n)
{
    ...
    if (n->span_id[0] != '\0') {
        PYDICT_SET_ITEM(x, "span_id", PyString_FromString(n->span_id));
    }
    if (n->parent_span_id[0] != '\0') {
        PYDICT_SET_ITEM(x, "parent_span_id",
                        PyString_FromString(n->parent_span_id));
    }
    PYDICT_SET_ITEM(x, "usid", PyInt_FromLong(n->usid));
    ...
    if (n->attributes != NULL && n->num_attributes > 0) {
        PyObject *attrs = PyDict_New();
        int i;
        for (i = 0; i < n->num_attributes; i++) {
            if (n->attributes[i].type == CONFD_PROGRESS_ATTRIBUTE_STRING) {
                PYDICT_SET_ITEM(
                    attrs, n->attributes[i].name,
                    PyString_FromString(n->attributes[i].value.string));
            } else if (n->attributes[i].type ==
                       CONFD_PROGRESS_ATTRIBUTE_NUMBER) {
                PYDICT_SET_ITEM(
                    attrs, n->attributes[i].name,
                    PyLong_FromUnsignedLong(n->attributes[i].value.number));
            }
        }
        PYDICT_SET_ITEM(x, "attributes", attrs);
    }
    if (n->links != NULL && n->num_links > 0) {
        PyObject *links = PyList_New(n->num_links);
        int i;
        for (i = 0; i < n->num_links; i++) {
            PyList_SetItem(
                links, i, newConfdProgressLink(&n->links[i]));
        }
        PYDICT_SET_ITEM(x, "links", links);
    }
...
*/
static void build_compaction_notification(
        PyObject *d, struct confd_compaction_notification *n)
{
    PyObject *x = PyDict_New();

    PYDICT_SET_ITEM(x, "dbfile",
                    PyLong_FromLong(n->dbfile));
    PYDICT_SET_ITEM(x, "type",
                    PyLong_FromLong(n->type));
    PYDICT_SET_ITEM(x, "fsize_start",
                    PyLong_FromUnsignedLongLong(n->fsize_start));
    PYDICT_SET_ITEM(x, "fsize_end",
                    PyLong_FromUnsignedLongLong(n->fsize_end));
    PYDICT_SET_ITEM(x, "fsize_last",
                    PyLong_FromUnsignedLongLong(n->fsize_last));
    PYDICT_SET_ITEM(x, "time_start",
                    PyLong_FromUnsignedLongLong(n->time_start));
    PYDICT_SET_ITEM(x, "duration",
                    PyLong_FromUnsignedLongLong(n->duration));
    PYDICT_SET_ITEM(x, "ntrans",
                    PyLong_FromLong(n->ntrans));

    PYDICT_SET_ITEM(d, "compaction", x);
}
...
EXT_API_FUN(_events_read_notification, EXT_API_FUN_EVENTS_READ_NOTIFICATION)
{
    ...
    confd_notification_type type = notif->n.type;

    PyObject *d = PyDict_New();

    PYDICT_SET_ITEM(d, "_notif", (PyObject*)notif);
    PYDICT_SET_ITEM(d, "type", PyLong_FromLongLong(type));
    ...
    else if (type == CONFD_NOTIF_COMPACTION) {
        build_compaction_notification(d, &notif->n.n.compaction);
    }
#ifdef CONFD_PY_PRODUCT_NCS
